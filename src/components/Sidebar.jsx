...
export default function Sidebar() {
  ...
  return (
    <div className={`h-full border-r bg-white transition-all duration-300 flex flex-col
      ${collapsed ? "w-16" : "w-64"}`}>

      {/* Collapse Button */}
      <button
        className="absolute top-4 right-[-14px] bg-white border rounded-full p-1 shadow 
      ...
      <div className="px-5 py-6 border-b">
        {!collapsed && (
          <h2 className="text-xl font-semibold tracking-tight">Notiq</h2>
        )}
      </div>
                  ...
                  {section.items.map(item => (
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        ...
                        ${isActive ? "bg-gray-100 shadow-sm" : "hover:bg-gray-100 hover:shadow"}`
                      }
                    >
                      {item.name}
                    </NavLink>
