...
function Login() {
  ...
  const sendEmailLink = async () => {
    ...
    try {
      ...
      alert("A sign-in link has been sent! Check your inbox.")
    } catch (err) {
  ...
  useEffect(() => {
    const cached = localStorage.getItem("cachedEmail")
  ...
  // ------------------------- PROVIDER LOGIN -----------------------------
  const loginProvider = async (name) => {
    try {
      await signInWithProvider(providers[name])
      localStorage.removeItem("cachedEmail")
      navigate("/dashboard")
    } catch (err) {
    ...
    <div className="min-h-screen flex bg-white">
      ...
      <div className="sm:w-screen flex-1 flex flex-col items-center justify-center p-10 md:p-16 bg-white shadow-lg">
        ...
        <div className="flex gap-4 w-full max-w-md ml-20">
          <button className="cursor-pointer bg-[#f5f5f5] px-6 h-20 rounded-xl text-gray-700 font-semibold hover:bg-white hover:shadow-md hover:scale-105 transition" onClick={() => loginProvider('google')}>
            <FcGoogle size={30} className="mx-auto mb-1" /> Google
          </button>

          <button className="cursor-pointer bg-[#f5f5f5] px-6 h-20 rounded-xl text-gray-700 font-semibold hover:bg-white hover:shadow-md hover:scale-105 transition" onClick={() => loginProvider('facebook')}>
            <FaFacebookF size={30} color="#1877F2" className="mx-auto mb-1" /> Facebook
          </button>

          <button className="cursor-pointer bg-[#f5f5f5] px-6 h-20 rounded-xl text-gray-700 font-semibold hover:bg-white hover:shadow-md hover:scale-105 transition" onClick={() => loginProvider('github')}>
            <FaGithub size={30} className="mx-auto mb-1" /> Github
          </button>
        </div>

        <div className="hidden md:flex px-16 gap-4 w-full max-w-md mt-4 ml-20">
          <button className="cursor-pointer bg-[#f5f5f5] px-6 h-20 rounded-xl text-gray-700 font-semibold hover:bg-white hover:shadow-md hover:scale-105 transition" onClick={() => loginProvider('discord')}>
            <FaDiscord size={30} className="mx-auto mb-1" /> Discord
          </button>
          
          <button className="cursor-pointer bg-[#f5f5f5] px-6 h-20 rounded-xl text-gray-700 font-semibold hover:bg-white hover:shadow-md hover:scale-105 transition" onClick={() => loginProvider('microsoft')}>
        ...
        <div className='flex flex-col w-full max-w-md text-gray-500 font-semibold'>
          ...
          <input type="email" placeholder='Enter your email address' className='bg-white border rounded-lg p-3 shadow-sm font-normal' value={email} onChange={(e) => setEmail(e.target.value)} />
          <button  onClick={sendEmailLink} className="btn bg-slate-600 p-3 text-center text-white shadow-md hover:bg-slate-800 mt-6 hover:scale-105 transform transition" > Continue </button>
          <p className='text-center font-normal text-gray-400 text-sm mt-2'>Use an organization email to easily collaborate with teammates </p>
        </div>
